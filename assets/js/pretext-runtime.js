(() => {
  ...
  function stepPreparedChunkLineGeometry(prepared, cursor, chunkIndex, maxWidth) {
    ...
    function maybeFinishAtSoftHyphen(segmentIndex) {
      ...
      if (pendingBreakKind !== "soft-hyphen" || pendingBreakSegmentIndex < 0)
        return null;
      const fitWidths = (_a = breakableFitAdvances[segmentIndex]) != null ? _a : null;
      if (fitWidths !== null) {
        const { fitCount, fittedWidth } = fitSoftHyphenBreak(fitWidths, lineW, maxWidth, lineFitEpsilon, discretionaryHyphenWidth);
        if (fitCount === fitWidths.length) {
          lineW = fittedWidth;
  ...
  function stepRichInlineLine(flow, maxWidth, cursor, collectFragment) {
    ...
    lineLoop: while (itemIndex < flow.items.length) {
      ...
      collectFragment == null ? void 0 : collectFragment(item, gapBefore, line.width + item.extraWidth, cloneCursor(textCursor), {
        segmentIndex: line.endSegmentIndex,
        graphemeIndex: line.endGraphemeIndex
      });
  ...
  // src/js/pretext/runtime.js
  var componentState = /* @__PURE__ */ new WeakMap();
  var selectors = {
  ...
  function scoreLines(lines, width, targetLines) {
    ...
    const orphanPenalty = widths.length > 1 && lastLineWidth < averageWidth * 0.58 ? (averageWidth - lastLineWidth) / Math.max(averageWidth, 1) : 0;
  ...
  function chooseBalancedLayout(prepared, availableWidth, lineHeight, targetLines, maxLines, minRatio) {
    const naturalWidth = Math.max(1, measureNaturalWidth(prepared));
    ...
    let bestResult = layoutWithLines(prepared, upperBound, lineHeight);
    ...
    for (let sampleIndex = 0; sampleIndex < 16; sampleIndex += 1) {
      ...
      const candidateResult = layoutWithLines(prepared, candidateWidth, lineHeight);
  ...
  function renderBalancedText(element) {
    ...
    const prepared = prepareWithSegments(sourceText, fontFromElement(element));
  ...
  function renderMeasuredText(element) {
    ...
    const prepared = prepare(sourceText, fontFromElement(element), options);
    const measurement = layout(prepared, availableWidth, lineHeight);
  ...
  function renderRichInlineText(element) {
    ...
    const lines = [];
    walkRichInlineLineRanges(prepared, availableWidth, (range) => {
  ...
  var runtime = {
    api: layout_exports,
    richInline: rich_inline_exports,
    components: {
      renderBalancedText,
      renderMeasuredText,
...
//# sourceMappingURL=pretext-runtime.js.map
